...
class Buffer:
    ...
    def estimate_norm_scaling_factor(self, batch_size, model, n_batches_for_norm_estimate: int = 100):
        ...
        for i in tqdm.tqdm(
            ...
            tokens = self.all_tokens[i * batch_size : (i + 1) * batch_size].to(model.cfg.device)
    ...
    def refresh(self):
        ...
        with torch.autocast("cuda", torch.bfloat16):
            ...
            self.first = False
            for _ in tqdm.tqdm(range(0, num_batches, self.cfg["model_batch_size"])):
                ...
                acts = torch.stack([cache_A[self.cfg["hook_point"]], cache_B[self.cfg["hook_point"]].to(cache_A[self.cfg["hook_point"]].device)], dim=0)
                acts = acts[:, :, :, :] 
